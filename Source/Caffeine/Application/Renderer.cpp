...
#include <iostream>
#define GL_DEBUG_ERROR(msg)													\
	{																		\
		GLenum err = glGetError();											\
		if(err != GL_NO_ERROR) {											\
			std::cout << "GL Error: " << err << " - " << msg << std::endl;	\
		}																	\
	}																		\
...
Renderer::~Renderer() {
	...
	// FIXME: Destroy stuff this isn't :(
}
...
void Renderer::reset()
{
	...
	setClearColor(0.3f, 1.0f, 0.0f, 1);
}
...
void Renderer::apply()
{
	...
	{
		GL_DEBUG_ERROR("Failed to use program!")
	...
	for(unsigned int i = 0; i < MAX_TEXTURES_UNIT; ++i)
	{
		...
		if(texID != NO_TEXTURE)
		{
			...
			GL_DEBUG_ERROR("Failed binding tex.");
		}
	}
	...
	if(m_selectedVertexBuffer != NO_VERTEX_BUFFER)
	{
		...
		GL_DEBUG_ERROR("Failed binding vertex buffer.");
		...
		if(m_selectedVertexFormat != NO_VERTEX_FORMAT)
		{
			...
			if(vertFormat->dirty)
			{
				for(unsigned int i = 0; i < vertFormat->numberOfAttrs; ++i)
				{
					...
					GL_DEBUG_ERROR("Failed applying attributes.");
				}
				
				// Keep dirty until shaders implimented
			}
		}
	}
	...
	if(m_selectedIndexBuffer != NO_INDEX_BUFFER) {
		...
		GL_DEBUG_ERROR("Failed binding index buffer.");
	}
}
