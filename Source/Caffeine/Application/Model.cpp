...
bool LoadOBJ(const std::string& filename, std::vector<CaffApp::Mesh> & meshes, const bool isGL)
{
	...
	if(file.is_open())
	{
		...
		while(getline(file, line))
		{
			...
			else if(type == "vn")
			{
				for(auto it = objTokens.begin() + 1; it != objTokens.end(); ++it)
				{
					normals.push_back(::atof((*it).c_str()));
				}
			}
...
{
	return LoadOBJ(filename, m_meshes, true);
}
...
Mesh & Model::getMesh(const uint16_t mesh) {
